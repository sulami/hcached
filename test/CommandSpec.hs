...
spec = do
  describe "Command Parser" $ do
    it "parses basic valid commands" $ do
      parse "set key 1 11 value value\n"
        `shouldBe` (Right $ SetCmd "key" 1 "value value")
      parse "get key\n" `shouldBe` (Right $ GetCmd "key")
      parse "delete key\n" `shouldBe` (Right $ DelCmd "key")

    it "parses set requests with multi-word values" $
      parse "set key 1 17 value more values\n"
        `shouldBe` (Right $ SetCmd "key" 1 "value more values")

    it "parses special characters in keys and values" $
      parse "set th!s-Key 1 12 S_x$#%@^&{}\"\n"
        `shouldBe` (Right $ SetCmd "th!s-Key" 1 "S_x$#%@^&{}\"")

    it "does not parse invalid content sizes" $ do
      parse "set key 1 4 value\n" `shouldSatisfy` isLeft
      parse "set key 1 6 value\n" `shouldSatisfy` isLeft
